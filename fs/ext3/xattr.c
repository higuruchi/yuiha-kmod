...
#define IHDR(inode, raw_inode)                                                \
  ((struct ext3_xattr_ibody_header *)((void *)raw_inode                       \
...
#define ea_idebug(inode, f...)                                                \
  do                                                                          \
    {                                                                         \
      printk (KERN_DEBUG "inode %s:%lu: ", inode->i_sb->s_id, inode->i_ino);  \
      printk (f);                                                             \
      printk ("\n");                                                          \
    }                                                                         \
  while (0)
#define ea_bdebug(bh, f...)                                                   \
  do                                                                          \
    {                                                                         \
      ...
      printk (KERN_DEBUG "block %s:%lu: ", bdevname (bh->b_bdev, b),          \
                      ...
                      void *buffer, size_t buffer_size)
{
  ...
  ea_idebug (inode, "name=%d.%s, buffer=%p, buffer_size=%ld", name_index, name,
  ...
  if (ext3_xattr_check_block (bh))
    {
    bad_block:
      ext3_error (inode->i_sb, __func__, "inode %lu: bad block " E3FSBLK,
                  inode->i_ino, EXT3_I (inode)->i_file_acl);
      error = -EIO;
      goto cleanup;
    }
                      ...
                      void *buffer, size_t buffer_size)
{
  ...
  if (error)
    goto cleanup;
  error = ext3_xattr_find_entry (&entry, name_index, name, end - (void *)entry,
                                 0);
  if (error)
    goto cleanup;
  size = le32_to_cpu (entry->e_value_size);
                ...
                void *buffer, size_t buffer_size)
{
  ...
  error = ext3_xattr_ibody_get (inode, name_index, name, buffer, buffer_size);
  if (error == -ENODATA)
    error
        = ext3_xattr_block_get (inode, name_index, name, buffer, buffer_size);
                         ...
                         char *buffer, size_t buffer_size)
{
  ...
  for (; !IS_LAST_ENTRY (entry); entry = EXT3_XATTR_NEXT (entry))
    {
      struct xattr_handler *handler = ext3_xattr_handler (entry->e_name_index);

      if (handler)
        {
          size_t size = handler->list (inode, buffer, rest, entry->e_name,
                                       entry->e_name_len);
          if (buffer)
            {
              if (size > rest)
...
ext3_xattr_ibody_list (struct inode *inode, char *buffer, size_t buffer_size)
{
  ...
  if (error)
    goto cleanup;
  error
      = ext3_xattr_list_entries (inode, IFIRST (header), buffer, buffer_size);
 ...
 */
static void
ext3_xattr_update_super_block (handle_t *handle, struct super_block *sb)
{
  ...
  if (i->value)
    {
      if (free < EXT3_XATTR_SIZE (i->value_len)
          || free < EXT3_XATTR_LEN (name_len) + EXT3_XATTR_SIZE (i->value_len))
        return -ENOSPC;
    }
      ...
      if (!s->here->e_value_block && s->here->e_value_size)
        {
          ...
          size_t size = EXT3_XATTR_SIZE (le32_to_cpu (s->here->e_value_size));
          ...
          while (!IS_LAST_ENTRY (last))
            {
              ...
              if (!last->e_value_block && last->e_value_size && o < offs)
                last->e_value_offs = cpu_to_le16 (o + size);
  ...
  if (EXT3_I (inode)->i_file_acl)
    {
      ...
      if (ext3_xattr_check_block (bs->bh))
        {
          ext3_error (sb, __func__, "inode %lu: bad block " E3FSBLK,
                      inode->i_ino, EXT3_I (inode)->i_file_acl);
          error = -EIO;
          goto cleanup;
        }
      ...
      error = ext3_xattr_find_entry (&bs->s.here, i->name_index, i->name,
                                     bs->bh->b_size, 1);
      ...
      if (header (s->base)->h_refcount == cpu_to_le32 (1))
        {
          ...
          if (error == -EIO)
            goto bad_block;
          if (!error)
            error = ext3_journal_dirty_metadata (handle, bs->bh);
          if (error)
            goto cleanup;
          goto inserted;
        }
  ...
  if (!IS_LAST_ENTRY (s->first))
    {
      ...
      if (new_bh)
      ...
      else
        {
          ...
          ext3_fsblk_t goal
              = ext3_group_first_block_no (sb, EXT3_I (inode)->i_block_group);
          ext3_fsblk_t block = ext3_new_block (handle, inode, goal, &error);
          if (error)
            goto cleanup;
  ...
  if (EXT3_I (inode)->i_state & EXT3_STATE_XATTR)
    {
      ...
      if (error)
        return error;
      /* Find the named attribute. */
      error = ext3_xattr_find_entry (&is->s.here, i->name_index, i->name,
...
retry:
  ...
  if (IS_ERR (handle))
  ...
  else
    {
      ...
      error = ext3_xattr_set_handle (handle, inode, name_index, name, value,
                                     value_len, flags);
...
again:
  ce = mb_cache_entry_find_first (ext3_xattr_cache, 0, inode->i_sb->s_bdev,
                                  hash);
  while (ce)
    {
      struct buffer_head *bh;
      ...
      if (!bh)
        {
          ext3_error (inode->i_sb, __func__, "inode %lu: block %lu read error",
                      inode->i_ino, (unsigned long)ce->e_block);
        }
      else if (le32_to_cpu (BHDR (bh)->h_refcount) >= EXT3_XATTR_REFCOUNT_MAX)
        {
          ea_idebug (
              inode, "block %lu refcount %d>=%d", (unsigned long)ce->e_block,
              le32_to_cpu (BHDR (bh)->h_refcount), EXT3_XATTR_REFCOUNT_MAX);
        }
 ...
 */
static inline void
ext3_xattr_hash_entry (struct ext3_xattr_header *header,
                       struct ext3_xattr_entry *entry)
{
  __u32 hash = 0;
  ...
  if (entry->e_value_block == 0 && entry->e_value_size != 0)
    {
      ...
      for (n = (le32_to_cpu (entry->e_value_size) + EXT3_XATTR_ROUND)
               >> EXT3_XATTR_PAD_BITS;
           n; n--)
        {
 ...
 */
static void
ext3_xattr_rehash (struct ext3_xattr_header *header,
                   struct ext3_xattr_entry *entry)
{
  struct ext3_xattr_entry *here;
  __u32 hash = 0;
...
{
  ext3_xattr_cache = mb_cache_create (
      "ext3_xattr", NULL,
      sizeof (struct mb_cache_entry)
          + sizeof (((struct mb_cache_entry *)0)->e_indexes[0]),
      1, 6);
  if (!ext3_xattr_cache)
    return -ENOMEM;
  return 0;
