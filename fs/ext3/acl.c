...
ext3_acl_from_disk (const void *value, size_t size)
{
  ...
  if (((ext3_acl_header *)value)->a_version != cpu_to_le32 (EXT3_ACL_VERSION))
    return ERR_PTR (-EINVAL);
...
ext3_acl_to_disk (const struct posix_acl *acl, size_t *size)
{
  ...
  *size = ext3_acl_size (acl->a_count);
  ext_acl = kmalloc (sizeof (ext3_acl_header)
                         + acl->a_count * sizeof (ext3_acl_entry),
                     GFP_NOFS);
  if (!ext_acl)
    return ERR_PTR (-ENOMEM);
              ...
              struct posix_acl *acl)
{
  ...
  error
      = ext3_xattr_set_handle (handle, inode, name_index, "", value, size, 0);
...
ext3_acl_chmod (struct inode *inode)
{
  ...
  if (!error)
    ...
    retry:
      handle
          = ext3_journal_start (inode, EXT3_DATA_TRANS_BLOCKS (inode->i_sb));
      if (IS_ERR (handle))
        {
...
static int
ext3_xattr_get_acl_access (struct inode *inode, const char *name, void *buffer,
                           size_t size)
{
  if (strcmp (name, "") != 0)
    return -EINVAL;
